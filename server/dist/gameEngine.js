...
import { getFibbageTruth, getHiddenTasks, getPromptForGame, getTriviaOptions, } from './games/loadContent.js';
import { getGame } from './games/registry.js';
let notifyRoomChange = () => { };
const roomTimers = new WeakMap();
export function setRoomNotifier(notifier) {
...
function startPhaseTimer(room, durationMs, onExpire) {
...
function beginRound(room, archetype) {
    ...
    if (gameMeta) {
        ...
        room.revealData = { ...(room.revealData || {}), presentation: pres };
    }
...
function transitionToInput(room) {
    const rules = rulesFor(room);
    const gameMeta = getGame(room.gameId);
    room.phase = 'input';
...
function assignSecretTasks(room) {
...
export function handleSubmit(room, playerId, value) {
    if (room.phase !== 'input')
        return false;
...
export function handleVote(room, playerId, targetId) {
    if (room.phase !== 'vote')
        return false;
...
function advanceAfterInput(room) {
    const game = getGame(room.gameId);
    if (!game)
        return;
...
function prepareVoting(room, archetype, revealMs, voteMs) {
    room.phase = 'reveal';
    ...
    if (archetype === 'fibbage') {
        const truth = room.revealData?.truth || 'truth';
...
function transitionToVote(room, voteMs) {
    room.phase = 'vote';
    const gameMeta = getGame(room.gameId);
    if (gameMeta) {
...
function finishVotePhase(room) {
    clearPhaseTimer(room);
    scoreRound(room);
    room.phase = 'results';
    startResultsPhase(room);
}
function startResultsPhase(room) {
    const rules = rulesFor(room);
    const gameMeta = getGame(room.gameId);
    if (gameMeta) {
...
function pairForVoting(entries) {
    const pairs = [];
...
function award(room, playerId, points) {
...
export function forceAdvance(room) {
    const rules = rulesFor(room);
    clearRoomTimers(room);
    room.timerEndsAt = undefined;
    room.timerStartedAt = undefined;
...
export function clearGameTimers(room) {
    clearRoomTimers(room);
}
