...
import { addPlayer, createRoom, markPlayerDisconnected, migratePlayerId, publicRoomState, reconnectPlayerByName, removePlayer, selectGame, } from './rooms.js';
...
function emitRoom(room) {
    ...
    for (const p of room.players) {
        if (p.connected)
            io.to(p.id).emit('room:update', publicRoomState(room, p.id));
    }
}
...
function applyGameToRoom(room, gameId) {
...
io.on('connection', (socket) => {
    ...
    socket.on('room:rejoin', ({ code, role, playerName, hostToken }, cb) => {
        ...
        if (role === 'host') {
            ...
            if (oldHostId !== socket.id)
                migratePlayerId(room, oldHostId, socket.id);
            currentRoom = room.code;
            playerId = socket.id;
            socket.join(room.code);
        ...
        if (room.phase !== 'lobby' && room.phase !== 'ended') {
            ...
            if (!player)
                return cb?.({ ok: false, error: 'Could not join (room full)' });
        }
        else {
            const player = addPlayer(room, socket.id, playerName.trim());
            if (!player)
                return cb?.({ ok: false, error: 'Could not join (room full or name taken)' });
        }
        currentRoom = room.code;
        playerId = socket.id;
        socket.join(room.code);
    ...
    socket.on('game:start', () => {
        const room = currentRoom ? rooms.get(currentRoom) : null;
        if (!room || room.hostId !== socket.id || !room.gameId)
            return;
    ...
    socket.on('room:end-hosting', (_data, cb) => {
        const room = currentRoom ? rooms.get(currentRoom) : null;
        if (!room || room.hostId !== socket.id) {
