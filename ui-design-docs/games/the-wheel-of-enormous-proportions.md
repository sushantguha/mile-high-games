# The Wheel of Enormous Proportions — UI Design

> Game ID: `the-wheel-of-enormous-proportions` · Archetype: `trivia` · Pack: Jackbox Party Pack 8
> **Inspiration (not a copy):** Wheel of Enormous Proportions — giant carnival wheel
...
### Prompt
- Phase label in accent color; Wheel Host emoji wiggles
- Prompt slides up inside `prompt-card` with glow border
- **Sound:** `prompt` chime (host only)

### Input
- Question slams down center; options appear as tiles
...
### Input
- Stacked answer buttons, selected state glows
