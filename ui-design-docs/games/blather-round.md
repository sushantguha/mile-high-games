# Blather 'Round — UI Design

> Game ID: `blather-round` · Archetype: `word-chain` · Pack: Jackbox Party Pack 7
> **Inspiration (not a copy):** Blather Round — owl word chain
...
|-------|-------|
| Accent | #a78bfa |
| Pattern | `game-pattern-wordchain` |
...
### Prompt
- Phase label in accent color; Owl emoji wiggles
- Prompt slides up inside `prompt-card` with glow border
- **Sound:** `prompt` chime (host only)

### Input
- Words chain across screen left-to-right; each link pops in
...
### Input
- Single text field, character limit visible, submit bounces
...
Typewriter tick, chain clink on submit
