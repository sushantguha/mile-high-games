# Monster Seeking Monster — UI Design

> Game ID: `monster-seeking-monster` · Archetype: `hidden-task` · Pack: Jackbox Party Pack 4
> **Inspiration (not a copy):** Monster Seeking Monster — dating app monsters
...
### Prompt
- Phase label in accent color; Dating Host emoji wiggles
- Prompt slides up inside `prompt-card` with glow border
- **Sound:** `prompt` chime (host only)

### Input
- Task envelope slides to player (client only); host shows group activity
...
### Input
- SECRET TASK card with warning border, then answer field
