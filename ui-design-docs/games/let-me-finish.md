# Let Me Finish — UI Design

> Game ID: `let-me-finish` · Archetype: `finish-sentence` · Pack: The Jackbox Naughty Pack
> **Inspiration (not a copy):** Let Me Finish — talk show teal mic
...
### Prompt
- Phase label in accent color; Tyler emoji wiggles
- Prompt slides up inside `prompt-card` with glow border
- **Sound:** `prompt` chime (host only)

### Input
- Sentence builds word by word on host
