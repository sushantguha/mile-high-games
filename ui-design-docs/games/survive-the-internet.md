# Survive the Internet — UI Design

> Game ID: `survive-the-internet` · Archetype: `text-transform` · Pack: Jackbox Party Pack 4
> **Inspiration (not a copy):** Survive the Internet — browser windows, Moxie
...
### Prompt
- Phase label in accent color; Moxie emoji wiggles
- Prompt slides up inside `prompt-card` with glow border
- **Sound:** `prompt` chime (host only)

### Input
- Fake UI windows stack and shuffle
...
### Input
- Transform prompt in monospace or social-post style
...
Notification ping, dial-up glitch
