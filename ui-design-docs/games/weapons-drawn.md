# Weapons Drawn — UI Design

> Game ID: `weapons-drawn` · Archetype: `draw-guess` · Pack: Jackbox Party Pack 8
> **Inspiration (not a copy):** Weapons Drawn — ninja dojo, red blades
...
- **Emoji decorations:** 🗡️ as host decoration with `host-pop` animation on phase entry
...
### Prompt
- Phase label in accent color; Sensei emoji wiggles
- Prompt slides up inside `prompt-card` with glow border
- **Sound:** `prompt` chime (host only)

### Input
- Drawing reveals with curtain pull or frame zoom
...
### Input
- Full-width canvas, chunky color/size tools, clear submit
...
Marker scratch loop (subtle), ta-da on submit
